...
@section('title', 'Edit Product - Drone FY')
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="product-form">
        ...
        <div class="form-group">
            ...
            <select 
                ...
                <option value="Under Maintenance" {{ old('status', $product->status) === 'Under Maintenance' ? 'selected' : '' }}>Under Maintenance</option>
            </select>
            @error('status')
                <span class="error">{{ $message }}</span>
            @enderror
        ...
        <div class="form-group">
            ...
            @if($product->product_image)
                <div class="current-image">
                    <img src="{{ asset($product->product_image) }}" alt="{{ $product->product_name }}" style="max-width: 200px; margin-bottom: 10px;">
